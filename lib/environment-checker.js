...
class EnvironmentChecker {
     ...
     */
    async checkAll() {
        console.error(chalk.blue('🔍 检查运行环境...'));
        ...
        // 总结
        if (allPassed) {
            console.error(chalk.green('✅ 所有环境检查通过'));
        } else {
            console.error(chalk.red('❌ 某些环境检查失败'));
    ...
    async checkPython() {
        try {
            ...
            if (passed) {
                console.error(chalk.green(`✅ Python: ${version.trim()}`));
            } else {
                console.error(chalk.red(`❌ Python: ${version.trim()} (需要 >= ${this.requirements.python.min})`));
            }

            return { passed, version: versionNumber, output: version };
        } catch (error) {
            console.error(chalk.red('❌ Python: 未安装或不在PATH中'));
    ...
    async checkTmux() {
        try {
            ...
            if (passed) {
                console.error(chalk.green(`✅ tmux: ${version.trim()}`));
            } else {
                console.error(chalk.red(`❌ tmux: ${version.trim()} (需要 >= ${this.requirements.tmux.min})`));
            }

            // tmux是可选的，即使版本不够也让它通过
            return { passed: true, version: versionNumber, output: version, optional: !passed };
        } catch (error) {
            console.error(chalk.yellow('⚠️  tmux: 未安装 (某些功能将受限)'));
    ...
    async checkNode() {
        try {
            ...
            if (passed) {
                console.error(chalk.green(`✅ Node.js: ${version.trim()}`));
            } else {
                console.error(chalk.red(`❌ Node.js: ${version.trim()} (需要 >= ${this.requirements.node.min})`));
            }

            return { passed, version: versionNumber, output: version };
        } catch (error) {
            console.error(chalk.red('❌ Node.js: 检查失败'));
            return { passed: false, error: error.message };
        }
    }
