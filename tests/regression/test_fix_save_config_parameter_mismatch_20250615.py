...
class TestSaveConfigParameterFix(unittest.TestCase):
    ...
    def test_save_config_with_merge_parameter(self):
        ...
        # 这应该成功工作，不抛出参数错误
        try:
            self.config_manager.save_config(test_config, merge=True)
    ...
    def test_save_config_with_merge_false(self):
        ...
        # 这应该成功工作，不抛出参数错误
        try:
            self.config_manager.save_config(test_config, merge=False)
    ...
    def test_save_config_parameter_name_consistency(self):
        ...
        sig = inspect.signature(self.config_manager.save_config)
    ...
    def test_regression_server_deletion_scenario(self):
        ...
        # 保存初始配置
        self.config_manager.save_config(initial_config, merge=False)
        ...
        # 这应该成功工作（这是导致原始错误的场景）
        try:
            self.config_manager.save_config(updated_config, merge=False)
